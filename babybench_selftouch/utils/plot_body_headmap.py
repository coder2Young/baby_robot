...
LOG_DIR = './results/self_touch/logs/PPO_18/' 
...
# 定义三个发育阶段 (时间步)
STAGES = {
    'Early Stage (0-1.5M steps)': (0, 1500000),
    'Middle Stage (1.5M-3M steps)': (1500000, 3000000),
    'Late Stage (3M-4M steps)': (3000000, 4000000)
}
...
def plot_stacked_bar_chart(aggregated_data, colormap_name, output_filename, grouping_threshold):
    ...
    ax.set_title('Evolution of Full Body Touch Distribution without Curiosity', fontsize=14, fontweight='bold')
...
def plot_non_head_distribution_chart(aggregated_data, colormap_name, output_filename, exclude_part, grouping_threshold):
    ...
    ax.set_title(f'Evolution of Touch Distribution (Excluding "{exclude_part}") without Curiosity', fontsize=14, fontweight='bold')
    ax.legend(title='Body Parts', bbox_to_anchor=(1.05, 1), loc='upper left')
    ax.set_ylim(0, 1)
    ax.tick_params(axis='x', labelsize=10, pad=10)
